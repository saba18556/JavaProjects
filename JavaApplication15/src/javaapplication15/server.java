...
public class server {
	...
	public static String toggle(String ch)
	{
		StringBuffer sb = new StringBuffer(ch);
		
		for(int i = 0; i < ch.length(); i++) {
			if(Character.isLowerCase(ch.charAt(i))) {
				sb.setCharAt(i, Character.toUpperCase(ch.charAt(i)));
			}
			else if(Character.isUpperCase(ch.charAt(i))) {
				sb.setCharAt(i, Character.toLowerCase(ch.charAt(i))); 
			}
  ...
  public static void main(String[] args) throws IOException{
    try{
      ...
      InetSocketAddress address = new InetSocketAddress("192.168.254.19", 32000);
      ...
      *  and store it in the corresponding buffer. */
      ...
      sendingDataBuffer = toggle(receivedData).getBytes();
     // sendingDataBuffer = Reverse(receivedData).getBytes();
